...
def product_scrape(scraper: Scraper, product_list: List[Tuple[int, float]]) -> List: # List here is a list of product models
...
def product_price_check(scraper: Scraper, product_list:  List[Tuple[int, float]]) -> List[Tuple[int, float]]:

    products = []
    for id, price in product_list:
        if scraper.price_changed(id):
